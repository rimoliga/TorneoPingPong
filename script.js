...
window.saveProfileChanges = async function () {
    ...
    if (!newName) return showToast("Nombre vacío", "error");
...
function renderBadges(p, played, won, streak, wallWins, gStats) {
    const cBadges = [{ icon: '🔥', name: 'On Fire', desc: 'Racha 2+', check: () => streak >= 2 }, { icon: '🛡️', name: 'Muro', desc: 'Rival <50%', check: () => wallWins >= 1 }, { icon: '⚔️', name: '1ra Sangre', desc: 'Ganar 1', check: () => won >= 1 }, { icon: '🔰', name: 'Debut', desc: 'Jugar 1', check: () => played >= 1 }];
    const gBadges = [{ icon: '🏆', name: 'Leyenda', desc: '5+ Copas', check: () => gStats.tourneys >= 5 }, { icon: '🦖', name: 'Veterano', desc: '20+ Partidos', check: () => gStats.played >= 20 }, { icon: '🎯', name: 'Sniper', desc: '>60% Win', check: () => gStats.played > 10 && (gStats.won / gStats.played) > 0.6 }, { icon: '🌟', name: 'Pro', desc: '1+ Copa', check: () => gStats.tourneys >= 1 }];
...
window.joinRoom = function () { const code = document.getElementById('joinCodeInput').value.trim().toUpperCase(); if (code.length < 2) return showToast("Código inválido", "error"); initAudio(); enterRoom(code); }
