...
window.saveProfileChanges = async function () {
    ...
    if (!newName) return showToast("Nombre vacÃ­o", "error");
...
function renderBadges(p, played, won, streak, wallWins, gStats) {
    const cBadges = [{ icon: 'ðŸ”¥', name: 'On Fire', desc: 'Racha 2+', check: () => streak >= 2 }, { icon: 'ðŸ›¡ï¸', name: 'Muro', desc: 'Rival <50%', check: () => wallWins >= 1 }, { icon: 'âš”ï¸', name: '1ra Sangre', desc: 'Ganar 1', check: () => won >= 1 }, { icon: 'ðŸ”°', name: 'Debut', desc: 'Jugar 1', check: () => played >= 1 }];
    const gBadges = [{ icon: 'ðŸ†', name: 'Leyenda', desc: '5+ Copas', check: () => gStats.tourneys >= 5 }, { icon: 'ðŸ¦–', name: 'Veterano', desc: '20+ Partidos', check: () => gStats.played >= 20 }, { icon: 'ðŸŽ¯', name: 'Sniper', desc: '>60% Win', check: () => gStats.played > 10 && (gStats.won / gStats.played) > 0.6 }, { icon: 'ðŸŒŸ', name: 'Pro', desc: '1+ Copa', check: () => gStats.tourneys >= 1 }];
...
window.joinRoom = function () { const code = document.getElementById('joinCodeInput').value.trim().toUpperCase(); if (code.length < 2) return showToast("CÃ³digo invÃ¡lido", "error"); initAudio(); enterRoom(code); }
...
window.callToPlay = async function (rIdx, mIdx) { const rounds = JSON.parse(JSON.stringify(gameState.rounds)); rounds[rIdx].matches[mIdx].status = 'ready'; await updateDoc(doc(db, window.DB_PATH_PREFIX, currentRoomId), { rounds }); showToast("ðŸ“¢ Â¡Jugadores llamados!"); }
...
window.startTournament = async function () {
    ...
    await updateDoc(doc(db, window.DB_PATH_PREFIX, currentRoomId), { rounds: [{ matches }], active: true, champion: null });
    playSound('win', 0.2);
}
...
function renderBracket() {
    ...
    gameState.rounds.forEach((r, rIdx) => {
        ...
        r.matches.forEach((m, mIdx) => {
            ...
            let statusBadge = ''; if (done) statusBadge = `<span class="badge-status status-done">FINALIZADO</span>`; else if (!m.isBye && (m.score1 > 0 || m.score2 > 0 || m.status === 'ready')) statusBadge = `<span class="badge-status status-live">â— EN JUEGO</span>`; else if (!m.isBye) statusBadge = `<span class="badge-status status-new">SIN INICIAR</span>`;

            // Special label for 3rd place
            let matchLabel = "";
            const totalRounds = Math.ceil(Math.log2(gameState.players.length));
            if (rIdx === totalRounds - 1 && gameState.rounds[rIdx].matches.length === 2) {
                if (mIdx === 0) matchLabel = '<div class="text-center text-[10px] text-yellow-400 font-bold mb-1 tracking-widest">ðŸ† GRAN FINAL</div>';
                if (mIdx === 1) matchLabel = '<div class="text-center text-[10px] text-slate-400 font-bold mb-1 tracking-widest">ðŸ¥‰ 3ER PUESTO</div>';
            }
