export function renderBracketView({
    ...
    (rounds || []).forEach((round, roundIdx) => {
        ...
        (round.matches || []).forEach((match, matchIdx) => {
            ...
            if (match.isBye) {
                html += `<div class="text-center py-4"><div class="font-bold text-white flex justify-center items-center gap-2">${getAvatar(match.p1, 32)} <span class="${match.p1 === currentUserIdentity ? "me-highlight" : ""} clickable-name" onclick="showProfileModal('${match.p1}')">${match.p1}</span></div><div class="text-xs text-slate-400 mt-1">BYE</div></div>`;
            } else {
                const n1 = playerMeta?.[match.p1]?.nickname || "";
                const n2 = playerMeta?.[match.p2]?.nickname || "";
                html += `<div class="flex justify-between items-center mb-2 ${p1W ? "text-yellow-400 font-bold" : "text-white"}"><span class="truncate w-32 text-sm"><div class="flex items-center gap-2">${getAvatar(match.p1)} <span class="${match.p1 === currentUserIdentity ? "me-highlight" : ""} clickable-name" onclick="showProfileModal('${match.p1}')">${match.p1} ${getPlayerBadges(match.p1)}</span></div>${n1 ? `<div class="text-[10px] text-slate-400/60 ml-8 italic">${n1}</div>` : ""}</span><span class="font-mono font-bold text-xl">${match.score1}</span></div>`;
                html += `<div class="flex justify-between items-center mt-2 ${p2W ? "text-yellow-400 font-bold" : "text-white"}"><span class="truncate w-32 text-sm"><div class="flex items-center gap-2">${getAvatar(match.p2 || "?")} <span class="${match.p2 === currentUserIdentity ? "me-highlight" : ""} clickable-name" onclick="showProfileModal('${match.p2}')">${match.p2 || "?"} ${getPlayerBadges(match.p2)}</span></div>${n2 ? `<div class="text-[10px] text-slate-400/60 ml-8 italic">${n2}</div>` : ""}</span><span class="font-mono font-bold text-xl">${match.score2}</span></div><div class="flex gap-2 mt-3">`;
                if (!done && match.p2) {
